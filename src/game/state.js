...
// origin and a players table, so the token lives under a common key. Existing
// sign-ins are carried over from the old per-game key.
const ACCOUNT_KEY = 'dle:account'

export function loadAccount() {
  try {
    const shared = JSON.parse(localStorage.getItem(ACCOUNT_KEY))
    if (shared) return shared
  } catch { /* fall through to the legacy key */ }
  const legacy = load('account', null)
  if (legacy) {
    saveAccount(legacy)
